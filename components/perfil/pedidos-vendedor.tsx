...
import { getOrdersBySeller, getProductById, updateOrder, getUserById } from "@/lib/file-storage"
import { FileText, Package2, Check, X } from "lucide-react"
import { useToast } from "@/components/ui/use-toast"

type PedidosVendedorProps = {
  userId: string
}

export function PedidosVendedor({ userId }: PedidosVendedorProps) {
  ...
  useEffect(() => {
    const fetchPedidos = async () => {
      try {
        const pedidosData = await getOrdersBySeller(userId)
        ...
        // Obter informações detalhadas dos produtos e compradores
  ...
  const handleUpdateStatus = async (pedidoId: string, newStatus: string) => {
    try {
      ...
      toast({
        ...
        description: `O pedido foi ${newStatus === "aceito" ? "aceito" : "cancelado"} com sucesso.`,
      })
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-8 flex flex-col items-center">
            ...
            <p className="text-gray-500">Você ainda não recebeu nenhum pedido.</p>
          </div>
        </CardContent>
      </Card>
    )
            ...
            <div key={pedido.id} className="border rounded-lg p-4">
              <div className="flex flex-wrap justify-between items-start gap-2 mb-4">
                <div>
                  ...
                  <p className="text-sm">
                    Cliente: <span className="font-medium">{pedido.comprador?.nome || "Cliente"}</span>
                  </p>
                </div>
                ...
                <DialogFooter>
                  {selectedPedido.status === "pendente" && (
                    <div className="flex gap-2">
                      ...
                      <Button
                        className="bg-green-500 hover:bg-green-600"
                        onClick={() => handleUpdateStatus(selectedPedido.id, "aceito")}
                      >
                        <Check className="mr-1 h-4 w-4" />
                        Aceitar Pedido
                      </Button>
